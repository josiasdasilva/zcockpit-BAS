...
this.getView().setModel(new JSONModel(json),'teste');
        ...
        initModelView: function () {
            this.getView().setModel(new JSONModel({ isNodeLevel: false, isProductLevel: false }), 'pedidoView');
        },
        setTableLevelNode: function(bIsNode){
            this.getView().getModel('pedidoView').setProperty('/isNodeLevel',bIsNode);
            this.getView().getModel('pedidoView').setProperty('/isNodeLevel',!bIsNode);
        },
        //FAFN - Begin
        ...
        onOpenPedidoProgressDialog: function (oEvent) {
            ...
            this.getView().setModel(new sap.ui.model.json.JSONModel({ text: '' }), "pedidoProgress");
            // instantiate dialog
            if (!this._pedidoBusyDialog) {
                this._pedidoBusyDialog = sap.ui.xmlfragment("dma.zcockpit.view.fragment.PedidoProgressBusyDialog", this);
                this.getView().addDependent(this._pedidoBusyDialog);
            }

            // open dialog
            jQuery.sap.syncStyleClass("sapUiSizeCompact", this.getView(), this._pedidoBusyDialog);
        ...
        initWSocket: function () {
            ...
            let hostLocation = window.location, socket, socketHostURI, webSocketURI;
